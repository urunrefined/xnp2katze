...
FILEH
file_open_rb(const OEMCHAR *path) { return fopen(path, "rb"); }

FILEH
file_create(const OEMCHAR *path) { return fopen(path, "wb+"); }
...
static BRESULT cnvdatetime(struct stat *sb, DOSDATE *dosdate,
                           DOSTIME *dostime) {
    struct tm *ftime;
...
short file_delete(const OEMCHAR *path) { return (short)unlink(path); }
...
short file_dircreate(const OEMCHAR *path) { return (short)mkdir(path, 0777); }
...
char *file_getcd(const OEMCHAR *filename) {
...
short file_delete_c(const OEMCHAR *filename) {
...
short file_attr_c(const OEMCHAR *filename) {
...
file_list1st(const OEMCHAR *dir, FLINFO *fli) {
    ...
    if (ret == NULL) {
        VERBOSE(("file_list1st: couldn't alloc memory (size = %d)",
                 sizeof(_FLISTH)));
...
static int euckanji1st(const OEMCHAR *str, int pos) {
    int ret;
    int c;
...
void file_catname(OEMCHAR *path, const OEMCHAR *filename, int maxlen) {
...
BOOL file_cmpname(const OEMCHAR *path, const OEMCHAR *path2) {
